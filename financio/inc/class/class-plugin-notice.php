...
class Plugin_Notice {
	...
	private function load_hooks() {
		...
		add_action( 'wp_ajax_financio_dismiss_notice', array( $this, 'dismiss_notice' ) );
	}
	...
	public function notice_install_plugin() {
		...
		if ( get_option( 'financio_dismiss_notice' ) ) {
			return;
		}
  ...
  '1x' => 'https://ps.w.org/gutenverse-companion/assets/icon-128x128.png?rev=3162415',
		...
		wp_register_style( 'financio-theme-notice', false );
		wp_enqueue_style( 'financio-theme-notice' );
		ob_start();
		?>
		
				.financio-simple-notice {
				
					padding: 24px !important; 
				
					border-left: 4px solid #007cba !important;
				}
				.financio-simple-notice p:first-child {
				
					margin-bottom: 20px; 
					margin-top: 0;
				}
				.financio-simple-notice p:last-of-type {
				
					margin-top: 0; 
					margin-bottom: 0;
				}
				.financio-simple-notice .financio-notice-title {
					font-size: 17px;
					font-weight: 600; 
					display: block;
					margin-bottom: 6px;
				}
				.financio-simple-notice .financio-notice-description {
					font-size: 13px;
					display: block;
				...
				.financio-simple-notice .button-primary {
					padding: 4px 16px !important;
				}

			
				.financio-simple-notice .notice-dismiss {
					background: none !important;
					box-shadow: none !important;
					opacity: 1;
				}
				
				.financio-simple-notice .notice-dismiss:before {
					color: #a7aaad;
					padding: 0; 
				}
				
				.financio-simple-notice .notice-dismiss:hover:before,
				.financio-simple-notice .notice-dismiss:focus:before {
					color: #c90000;
				}
		...
		wp_add_inline_style( 'financio-theme-notice', $custom_style );
		wp_register_script(
			'financio-theme-notice',
			'',
			array( 'wp-api-fetch' ),
			'1.0.0',
			true
		...
		wp_enqueue_script( 'financio-theme-notice' );
		...
		function sequenceInstall(plugin, pluginsInstalled) {
			return new Promise((resolve, reject) => {
				...
				const needUpdate = plugin.installed
					? FINANCIOPluginUtils.isVersionGreater(plugin.req_version, pluginsInstalled[`${path}.php`].Version)
					: false;
		...
		document.addEventListener('DOMContentLoaded', () => {
			const notice = document.querySelector('.notice.is-dismissible.financio-simple-notice');
				...
				setTimeout(() => {
					...
					if (dismissBtn) {
						dismissBtn.addEventListener('click', (e) => {
							e.preventDefault();
							jQuery.post(ajaxurl, {
								action: 'financio_dismiss_notice',
								_ajax_nonce: nonce
							});
						});
					}
			...
			button.addEventListener('click', function (e) {
				...
				button.innerHTML = `<?php esc_html_e( 'Installing...', 'financio' ); ?>`;
				...
				if (!hasFinishClass) {
					...
					sequence.then(() => {
						...
						button.innerHTML = `<?php esc_html_e( 'Install Failed, Try Again', 'financio' ); ?>`;
		...
		wp_add_inline_script( 'financio-theme-notice', $custom_script );

		?>
		<div class="notice notice-info is-dismissible financio-simple-notice" data-nonce="<?php echo esc_attr( wp_create_nonce( "financio_dismiss" ) ); ?>">
				<p>
					<strong class="financio-notice-title"><?php esc_html_e( "Thank you For Installing Financio Theme", "financio" ); ?></strong>
					<span class="financio-notice-description">
						<?php esc_html_e( "Unlock the full potential of your website with the recommended plugins.", "financio" ); ?>
						<br/>
						<?php esc_html_e( "Activate it to explore exclusive extensions, ready-to-use demo templates, and powerful features that make building your site easier and more enjoyable.", "financio" ); ?>
					</span>
				</p>
				<p>
					<a href="#" class="button button-primary" id="gutenverse-install-plugin"><?php echo esc_html__( "Install Recommended Plugins", "financio" ); ?></a>
				...
				<p class="installing-warning" style="font-size: 12px; color: #666; display: none; margin-top: 5px;">
					<i><?php esc_html_e( "Don\'t refresh the page when installing recommended plugins", "financio" ); ?></i>
				</p>
			</div>
		<?php
	}
	...
	public function dismiss_notice() {
		check_ajax_referer( 'financio_dismiss' );

		if ( ! get_option( 'financio_dismiss_notice' ) ) {
			update_option( 'financio_dismiss_notice', true );
		}
