...
class Upgrader {
	...
	public function upgrader_index_close() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'financio_upgrade_index' ) ) {
	...
	public function upgrader_page_close() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'financio_upgrade_notice' ) ) {
	...
	public function upgrader_page_content_close() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'financio_content_upgrade_notice' ) ) {
			...
			<script>
				(function($) {
					...
					$('.theme-upgrade-notice.index-upgrade .close-notification').on('click', function( event ) {
						...
						var nonce = '<?php echo esc_html( wp_create_nonce( 'financio_upgrade_index' ) ); ?>';
					...
					<script>
						(function($) {
							$('.theme-upgrade-notice.page-content-upgrade .close-notification').on('click', function( event ) {
								event.preventDefault();
								var nonce = '<?php echo esc_html( wp_create_nonce( 'financio_content_upgrade_notice' ) ); ?>';
...
<!-- /wp:paragraph -->
							<div class="themes-upgrade-action">
								<a class='close-notification' href="#">
									...
									<?php esc_html_e( 'Close notification', 'financio' ); ?>
								</a>
					...
					<script>
						(function($) {
							$('.theme-upgrade-notice.page-upgrade .close-notification').on('click', function(event) {
								event.preventDefault();
								var nonce = '<?php echo esc_html( wp_create_nonce( 'financio_upgrade_notice' ) ); ?>';
